...
describe("Create Group", () => {
  ...
  it("should be able to create a group", async () => {
    ...
    await inMemoryMaterialsRepository.createMany([
      makeMaterial(
        { code: 10000, description: "SOME TEST MATERIAL" },
        new UniqueEntityID("some-test-material-id"),
      ),
      makeMaterial(
        { code: 10001, description: "SOME TEST MATERIAL 2" },
    ...
    await inMemoryCablesRepository.createMany([
      makeCable({}, new UniqueEntityID("some-local-cable-id")),
    ]);
    ...
    const result = await sut.execute({
      ...
      items: [
        ...
        },
        {
          type: "cableConnector",
          quantity: 5,
          localCableId: "some-local-cable-id",
          addByPhase: 3,
      ...
      expect(inMemoryGroupItemsRepository.items[3]).toEqual(
        expect.objectContaining({
          props: expect.objectContaining({
            ...
            addByPhase: 3,
            oneSideConnector: false,
            localCableId: expect.objectContaining({
              value: "some-local-cable-id",
            }),
          }),
        }),
      );
  ...
  it("should not be able to create a group with negative value for poleScrew lengthAdd", async () => {
    // Removemos a parte do conector deste teste de erro, já que ele não tem mais valores numéricos
    const result = await sut.execute({
      name: "Some Group",
      description: "Some Group Description",
      tension: "low",
  ...
  it("should not be able to create a group with unexisting local cable id", async () => {
    const result = await sut.execute({
      name: "Some Group",
      description: "Some Group Description",
      tension: "low",
      items: [
        {
          type: "cableConnector",
          quantity: 5,
          localCableId: "unexisting-cable-id",
        },
      ],
    });
    ...
    if (result.isLeft()) {
      ...
      expect(result.value.message).toBe(
        "Local Cables with missing IDs: unexisting-cable-id",
      );
    }
  });
  ...
  it("should not be able to create a group with invalid tension", async () => {
    const result = await sut.execute({
      ...
      description: "Some Group 2",
      tension: "invalid",
      items: [{ type: "poleScrew", quantity: 1, lengthAdd: 10 }],
    });
  ...
  it("should not be able to create a group with invalid materialId", async () => {
    const result = await sut.execute({
      ...
      items: [
        {
          type: "material",
          materialId: "unexisting-material-id",
          quantity: 10,
        },
      ],
    });
    ...
    if (result.isLeft()) {
      expect(result.value).toBeInstanceOf(ResourceNotFoundError);
      expect(result.value.message).toBe(
        "Materials with missing IDs: unexisting-material-id",
      );
    }
  });
});
