...
describe("Create Bulk Of Pole Screws (E2E)", () => {
  ...
  test("[POST] /pole-screws/bulk", async () => {
    ...
    const response = await request(app.getHttpServer())
      ...
      .send({
        poleScrews: [
          ...
          {
            code: 54321,
            description: "PARAFUSO DE POSTE 20MM",
            unit: "UND",
            lengthInMM: 300,
          },
        ],
    ...
    const poleScrew1 = await poleScrewsRepository.findByCode(12345);
    const poleScrew2 = await poleScrewsRepository.findByCode(54321);

    expect(poleScrew1).toBeTruthy();
    expect(poleScrew2).toBeTruthy();
  });
});
